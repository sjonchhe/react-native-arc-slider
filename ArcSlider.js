...
import Svg, {Path,Circle,G,LinearGradient, Defs, Stop} from 'react-native-svg'
...
class ArcSlider extends Component {
  ...
  static defaultProps = {
    ...
    width: 375,
    height: 375,
    startColor: 'rgb(238,167,1)',
    endColor: 'rgb(255,88,2)',
    strokeWidth: 5,
    thumbR:13,
    thumbColor: '#fdc357',
    thumbWrapperR: 18,
  ...
  cartesianToPolar(x,y){
    ...
    return Math.round((Math.atan((cy - y)/(cx - x)))/(Math.PI/180)) + (x>cx ? 270 : 90)
  }
  ...
  render(){
    ...
    const {
        width,height,
        thumbColor, thumbWrapperColor,startColor, endColor, maximumTrackColor, 
        strokeWidth, thumbR, thumbWrapperR,
        min, max} = this.props
        ...
        <Svg 
          ...
          width={width} height={height}>
            ...
            <Path 
              ...
              fill='none'
              d={`M${startCoord.x} ${startCoord.y} A ${r} ${r} 0 ${max>180 + min ?1:0} 1 ${maxCoord.x} ${maxCoord.y}`}
            />
            {/* minimumTrack */}
            <Path 
            ...
            />
            <Defs>
              <LinearGradient id="minTrackColor" x1="0" y1="0" x2="170" y2="0">
                  <Stop offset="0" stopColor={startColor} stopOpacity="1" />
                  <Stop offset="1" stopColor={endColor} stopOpacity="1" />
              </LinearGradient>
            </Defs>
            <G
              x={endCoord.x-7.5} y={endCoord.y-7.5}
              {...this._panResponder.panHandlers}
            >
              <Circle 
                cx={7.5} cy={7.5}
                r={thumbWrapperR + 50} fill="rgba(0,0,0,0)"/>
              <Circle 
                cx={7.5} cy={7.5}
                r={thumbWrapperR} fill={thumbWrapperColor}/>
              <Circle 
                cx={7.5} cy={7.5}
                r={thumbR} fill={thumbColor}/>
            </G>
        </Svg>
        {this.props.children}
      </View>
